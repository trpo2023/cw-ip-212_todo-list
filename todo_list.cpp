...
void saveTasksToFile(const ToDoList& todo, const string& filename)
{
    ...
    if (file.is_open())
    {
        ...
        {
            file << task.name << "," << task.is_done << endl;
        }
        file.close();
        cout << "Tasks are saved to file: " << filename << endl;
    }
    else
    {
...
void loadTasksFromFile(ToDoList& todo, const string& filename)
{
  ...
  if (file.is_open())
  {
    ...
    while (getline(file, line))
    {
      ...
      {
        string taskName = line.substr(0, commaIndex);
        bool isDone = stoi(line.substr(commaIndex + 1));
        todo.add_task(Task(taskName));
        if (isDone)
        {
          int taskIndex = todo.tasks.size() - 1;
          todo.mark_task_done(taskIndex);
        }
      }
    }
    file.close();
    cout << "Tasks are loaded from file: " << filename << endl;
  }
  else
  {
    cout << "Failed to open file: " << filename << endl;
...
int main()
{
  ...
  loadTasksFromFile(todo, "tasks.csv");
...
while (true) 
  {
      ...
      switch (choice) 
      {
          ...
          case 7: 
          {
              ...
              saveTasksToFile(todo, "tasks.csv");
